  ...
  return (
    <div className="overflow-hidden rounded-xl border border-zinc-200/60 bg-zinc-100/40 dark:border-zinc-800/60 dark:bg-zinc-900/40">
      <div className="px-6 pt-6 pb-4">
        <p className="mb-1 font-semibold text-sm text-zinc-400 uppercase tracking-widest dark:text-zinc-500">{label}</p>
        <p className="text-zinc-500 dark:text-zinc-400">{description}</p>
      </div>
      <div className="border-zinc-200/60 border-t dark:border-zinc-800/60">
        {variants.length > 1 && (
          <div className="flex gap-1 border-zinc-200/60 border-b px-4 py-2 dark:border-zinc-800/60">
            {variants.map((v, i) => (
              <button
                key={v.name}
                type="button"
        ...
        <div className="flex border-zinc-200/60 border-b px-4 dark:border-zinc-800/60">
        ...
        {tab === "preview" ? (
          <div className="p-8">{variants[variantIndex].preview}</div>
        ) : (
          <div className="max-h-96 w-0 min-w-full overflow-auto bg-zinc-950 dark:bg-zinc-900/80">
            <ShikiHighlighter
              language="tsx"
              theme="github-dark"
              showLineNumbers={true}
              showLanguage={true}
              className="text-sm [&>pre]:bg-transparent! [&>pre]:p-4 [&>pre]:leading-relaxed"
            >
